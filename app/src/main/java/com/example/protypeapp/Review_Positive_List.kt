...
class Review_Positive_List : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_review_not_negative_list)
        reviewRecyclerView = findViewById(R.id.reviewRecyclerView) // Убедитесь, что ID совпадает с макетом
            ...
            override fun onResponse(call: Call<List<Review>>, response: Response<List<Review>>) {
                if (response.isSuccessful) {
                    ...
                    if (response.code() == 401) {
                        ...
                        Toast.makeText(this@Review_Positive_List, errorMessage, Toast.LENGTH_SHORT).show()
                    }
                }
            }

            override fun onFailure(call: Call<List<Review>>, t: Throwable) {
                Toast.makeText(this@Review_Positive_List, "Ошибка сети: ${t.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
